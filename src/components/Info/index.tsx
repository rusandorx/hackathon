...
const infoData = [
  ...
  },
  {
    ...
    description: "Задайте диапазон IP-адресов через двоеточие",
    example: "192.168.0.0:192.168.0.255",
  },
  {
    id: 3,
    type: "CIDR-нотация",
    description: "Используйте CIDR-нотацию для сканирования подсетей",
    ...
    <div className="overflow-hidden mt-8 flex items-center bg-white rounded-md shadow p-2">
      <table className="info__table w-full table-fixed">
        <thead className="text-lg border-b">
          <motion.tr {...inView} variants={fadeIn("right", "spring", 0, 1.2)}>
        ...
        <tbody className="text-center bg-white">
          {infoData.map((item) => {
            return (
              <motion.tr
                {...inView}
                variants={fadeIn("left", "spring", 0.1 * item.id, 1.2)}
                key={item.id}
              >
                <td>
  ...
  return (
    <section className="mt-24 mb-12 text-secondary">
      <div className="container mx-auto px-4 lg:px-8 xl:px-16">
        ...
        >
          Воспользуйтесь настройками и введите желаемый адрес в одном из
          форматах, предложенных ниже:
        </motion.p>
