...
const Converter = () => {
  ...
  const ipChangeHandler = (e: ChangeEvent<HTMLInputElement>): void => {
    // setState(prev => ({ ...prev, ip: e.target.value }))
    if (!ipRef.current || !chechIpPattern(ipRef.current.value)) return;
    submitIp(ipRef.current?.value);
  };

  const domenChangeHandler = (e: ChangeEvent<HTMLInputElement>): void => {
